...
Lithium relies on the [Forge Loader](https://files.minecraftforge.net/). Users should select Forge for either the Minecraft launcher (client) or
the dedicated server (server) depending on their needs.
Once you have installed Forge, place the Lithium .jar in the `mods` folder generated by Forge.
